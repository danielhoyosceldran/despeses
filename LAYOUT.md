...
- **Sections**:
  - **Categories / Tags** (preferred): `AppCard.large` donut (`DonutChart`) with the total in the hole; Categories supports drill-down — tap slice/legend pushes a real navigation route (`_CategoryDrillScreen`, own `AppTopBar` titled with the entered category) so the OS back gesture/button pops one level; the pushed screen also shows a breadcrumb Row with circular back for tap-to-pop. Legend rows below (`LegendRow`). Leaf-only, so no "direct" slice.
